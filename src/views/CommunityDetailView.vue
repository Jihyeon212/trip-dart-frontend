...
function routeData(): Location[] { if (!post.value?.route_data) return []; if (Array.isArray(post.value.route_data)) return post.value.route_data; try { return JSON.parse(post.value.route_data) as Location[] } catch { return [] } }
...
<template>
  <div class="page container detail-page"><StatusPanel v-if="loading" type="loading" message="게시글을 불러오고 있어요." /><StatusPanel v-else-if="error" type="error" :message="error" action-label="게시글 다시 불러오기" @action="load" />
    <article v-else-if="post" class="post-detail"><header><span :class="['post-badge', post.post_type]">{{ POST_TYPE_LABELS[post.post_type] }}</span><h1>{{ post.title }}</h1><div><span>{{ post.nickname }}</span><time>{{ new Date(post.created_at).toLocaleString('ko-KR') }}</time></div></header><div class="post-content">{{ post.content }}</div><section v-if="routeData().length" class="shared-route"><h2>함께 공유된 여행 코스</h2><ol><li v-for="location in routeData()" :key="location.contentid"><span>{{ location.categoryLabel }}</span><strong>{{ location.title }}</strong><small>{{ location.address }}</small></li></ol></section><footer><RouterLink to="/community" class="button secondary">커뮤니티 목록으로</RouterLink><div><button class="button ghost" type="button" @click="modal = 'edit'; modalError = ''">게시글 수정</button><button class="button danger-outline" type="button" @click="modal = 'delete'; modalError = ''">게시글 삭제</button></div></footer></article>
